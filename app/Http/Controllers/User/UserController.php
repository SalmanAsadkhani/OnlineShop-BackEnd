...
class UserController extends Controller
{
    ...
    public function store(UserStoreRequest $userStoreRequest , User $user)
    {
        $user = $user->create($userStoreRequest->all());

        if ($user) {

            return response()->json([
                'message' => 'کاربر با موفقیت ثیت نام کرد' ,
                'data' => new UserResource($user)
            ]);
        }
        else {
            return response()->json([
                'message' => 'خطایی در مراحل ثبت نام رخ داد'
            ]);
        }
